            ...
            let message: [Message] = value.compactMap({dictionary in
                ...
                let media = mediaItem(url: url, image: nil, placeholderImage: UIImage(systemName: "play")!, size: CGSize(width: 300, height: 300))
                    ...
                    guard let latitude = Double(locationComponent[0]),
                        let logitude = Double(locationComponent[1]) else{
                            return nil
                    }
                    let sellectedLocation = Location(location: CLLocation(latitude: latitude, longitude: logitude), size: CGSize(width: 300, height: 300))
                    kind = .location(sellectedLocation)
    ...
    public func sendMessage(to conversationId: String, otherUserEmail: String, name: String, newMessasge: Message, completion: @escaping (Bool) -> Void){
        ...
        database.child("\(conversationId)/message").observeSingleEvent(of: .value, with: {[weak self] snapshot in
            ...
            case .location(let targetLocation):
                messageMedia = "\(targetLocation.location.coordinate.latitude),\(targetLocation.location.coordinate.longitude)"
            case .emoji(_):
                break
            case .audio(_):
                break
                        ...
                        strongSelf.database.child(("\(otherUserEmail)/conversations")).observeSingleEvent(of: .value, with: {snapshot in
                            ...
                            if var otherUserConversation = snapshot.value as? [[String: Any]] {
                                ...
                                if var targetConversation = targetConversation{
                                    ...
                                    // faild to find current collection
                                    let conversationData: [String : Any] = [
                                        "id": conversationId,
                                        "other_user_email": DatabaseManger.safeEmail(emailAddress: currentUser),
                                        "name": currentUserName,
                                        "lates_message": updateMessageValue
                                    ]
                                    otherUserConversation.append(conversationData)
                                    databaseEntryConversation = otherUserConversation
                                }
